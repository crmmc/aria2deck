...
| 约束 | 说明 |
|------|------|
| 背景不透明 | 所有浮层（Modal、下拉面板、悬浮窗、Tooltip 等）内容区域必须使用不透明背景 (`background: #fff !important`)，不使用半透明玻璃效果 |
| 覆盖 .card 透明度 | 浮层若复用 `.card` 类，必须用 `!important` 覆盖其半透明背景，包括 `:hover` 状态 |
| 遮罩层 | Modal 遮罩使用 `rgba(0,0,0,0.5)` 半透明黑色 |
| 点击遮罩关闭 | 点击遮罩层应关闭弹窗 |
| 阻止冒泡 | 弹窗内容区域点击不应触发关闭 |
