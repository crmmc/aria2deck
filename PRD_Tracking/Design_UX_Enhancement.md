...
| 约束 | 说明 |
|------|------|
| 背景色 | 弹窗必须使用**不透明背景**（如 `#ffffff`），禁止使用透明或半透明背景 |
| 对比度 | 字体颜色与背景色的对比度必须足够高，确保用户能清晰阅读 |
| 按钮样式 | 危险操作按钮使用红色背景 + 白色文字（`background: #ff3b30; color: white`） |
| 遮罩层 | 弹窗遮罩使用半透明黑色（`rgba(0,0,0,0.5)`），与弹窗内容形成对比 |

**示例**:
```css
/* 弹窗容器 */
.modal {
  background: #ffffff;  /* 不透明白色背景 */
