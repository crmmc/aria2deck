...
async def test_failed_retry_creates_new_attempt_ids(temp_db: str) -> None:
    ...
    first = await create_download_task(
        user_id=user["id"],
    ...
    second = await create_download_task(
        user_id=user["id"],
        quota_bytes=user["quota_bytes"],
        uri="https://example.com/new-attempt.bin",
        resource_key="http:new-attempt",
    ...
    old_global = await get_global_download_by_id(int(global_download_id_of(first)))
    new_global = await get_global_download_by_id(int(global_download_id_of(second)))
...
async def test_terminal_global_download_never_resurrected(temp_db: str) -> None:
    ...
    created = await get_global_download_by_id(int(global_download_id_of(task)))
...
async def test_completed_other_user_attaches_store_file(temp_db: str) -> None:
    ...
    assert global_download_id_of(task) == completed["id"]
